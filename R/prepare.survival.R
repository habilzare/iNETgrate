prepare.survival <- function(clinical, patientIDCol="bcr_patient_barcode",
    ...
    if(!is.null(riskCatCol)){
        ...
        if(!(any(c(is.null(riskHigh), is.null(riskLow))))){
            ...
            message.if(cat("Risk categories will be reassigned as High, Int and Low as ",
    ...
    if(length(toKeep) < nrow(survival)){
        cat(nrow(survival) - length(toKeep), 
