survival.analysis <- function(survival, event="Dead", Labels, Data=NULL, 
    ...
    for(mu in mus){
        ...
        if(doCox){
            bestModules <- coxAnalysis(dataCategory="allSamples",
                                       ...
                                       inputMatrix=coxData,
                                       glmOutPath=outPath, verbose=verbose-1)
    ...
    message.if(print(bestPvalues), verbose=verbose-1)
    
    ## Timing:
    timeTaken <- Sys.time() - starTime
    message.if(me=cat("Survival analysis took:", timeTaken, attr(timeTaken,"units"), "\n"), 
               verbose=verbose-3) 
    ## Output:
    return(list(mus=mus,
