elect.genes <- function(genExpr, dnam, survival, savePath, event="Dead", locus2gene,
    ...
    ## This is a wrapper function that executes, filter.low.cor and plotLoci.num 
    ...
    filteredExpr <- filter.low.cor(Data=genExpr, survival=survival,
    ...
    filteredDnam <- filter.low.cor(Data=dnam, survival=survival,
    ...
    if(!is.null(savePath)){
        ...
        plotted <- plotLoci.num(locus2gene=locus2gene, selectedLoci=filteredDnam$selectedList,
                                plotFile=plotFile, doAddTitle=doAddTitle)
    }

    ## Compute union:
    unionSet <- compute.union(Genes=rownames(genExpr),
