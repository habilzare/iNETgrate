compute.union <- function(Genes, selectedGenes, loci, selectedLoci, locus2gene,
    ...
    ##Timing:
    starTime <- Sys.time()
    message.if(me=cat("Identifying a union gene set:", format(starTime, usetz=TRUE)),
               verbose=verbose-3)

    ## QC:
    if(length(intersect(locus2gene[ ,genesColName], selectedGenes))==0){
        stop(paste("selectedGenes do not overlap with", genesColName,
    ...
    message.if(cat("unionGenes set size:", length(unionGenes), "\n"),
               verbose=verbose)
    ...
    message.if(me=cat("Computing union set took:", timeTaken, 
