compute.eigenloci <- function(dnam, locus2gene, geNames=NULL, Labels,
    ...
    ## locus2oneGene: A matrix as explained in compute.union.R.
    ...
    if(any(rownames(locus2gene) != locus2gene[, lociColName]))
        stop(paste("Row names of locus2gene do not agree with its",
                   lociColName,"column!"))
         
    ## One locus - one gene mapping
    createdLocusGene <- create.locusGene(locus2gene=locus2gene,
    ...
    numG <- length(geNames)
    message.if(cat("For every one of the", numG, "genes... \n"), verbose=verbose)
    eigenloci <- c()
    lociPigen <- list()
    usefuLoci <- list()
    ...
    for(i1 in seq_len(numG)){
        if((i1 %% 100) == 0)
            message.if(cat(i1, "of", numG, "\n"), verbose=verbose)
        g1 <- geNames[i1]
        loci1 <- gene2loci[[g1]]
        if(length(loci1)==0)
            stop(paste("No locus is available for", g1))
        if((i1 %% 100) ==0){
            message.if(cat(g1, i1, "th out of", numG, ". Number of its loci:", 
                           length(loci1), "\n"), verbose=verbose)
        }
        ##trim
        found <- find.core(Data=t(dnam[loci1, , drop=FALSE]), Labels=Labels, 
    ...
    distanceToTss <- compute.distance.to.tss(usefuLoci=usefuLoci,
                                             locus2oneGene=locus2oneGene,
                                             genesColName=genesColName,
                                             lociColName=lociColName,
                                             coordinatesColName=coordinatesColName,
                                             verbose=verbose-1)
    plotLoci.gene(distanceToTss=distanceToTss$distanceToClosesTss, plotFile=plotFileGene)
    ...
    if(!is.null(dnamGene)){
        ...
        distanceToTssDnam <- compute.distance.to.tss(usefuLoci=usefuLociDnam,
                                                     locus2oneGene=locus2oneGene,
                                                     genesColName=genesColName,
                                                     lociColName=lociColName,
                                                     coordinatesColName=coordinatesColName)
        plotFileDnam <- file.path(plotPath,"usefuLoci_dnamGenes.png")
        plotLoci.gene(distanceToTss=distanceToTssDnam$distanceToClosesTss, plotFile=plotFileDnam)
