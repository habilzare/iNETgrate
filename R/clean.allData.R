clean.allData <- function(genExpr, genExprSampleInfo, rawDnam, rawDnamSampleInfo=NULL, 
    ...
    ## This is a wrapper function that executes preprocess.dnam,
    ## prepare.survival functions, and sets sample2patient mapping for geneExpr,
    ...
    processedDnam <- preprocess.dnam(rawDnam=rawDnam,
                                     rawDnamSampleInfo=rawDnamSampleInfo, 
                                     savePath=savePath,  annLib=annLib,
                                     verbose=verbose)

    ## prepare survival data
    message.if(me="Processing clinical data for survival analysis...",
               verbose=verbose)
    survival <- prepare.survival(clinical=clinical, patientIDCol=patientIDCol,
    ...
    message.if(me="Sample to patient mapping for gene expression data ...",
               verbose=verbose)
    s2pExpr <- sample.to.patient(sampleInfo=genExprSampleInfo, sampleCol=sampleCol,
    ...
    s2pDnam <- sample.to.patient(sampleInfo=processedDnam$sampleInfo, 
    ...
    message.if(me=cat("Cleaning all data took:", timeTaken, attr(timeTaken,"units"), "\n"), 
               verbose=verbose-3)
