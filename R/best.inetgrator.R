best.inetgrator <- function(bestPvalues, usefuLoci, lociPigen, netPath, verbose=0){
    ...
    message.if("Making best inetgrator...", verbose=verbose)
    ...
    message.if(me=cat("The best mu value based on p-values(",
                      bestPvalue, ") is: ", mu,"\n"), verbose=verbose-1)
    message.if(me=cat("The best modules are: ", as.character(modelName), "\n"),
               verbose=verbose-1)
    ...
    for(m1 in features){
        ...
        message.if(cat("Loading pigengene data for \n", file1, "\n"),
                   verbose=verbose-2)
    ...
    inetgrator <- compute.inetgrator(mu=mu, usefuLoci=usefuLoci,
    ...
    message.if(cat("Finished data processing at", format(Sys.time(), usetz=TRUE),"\n"),
               verbose=verbose-3)
    return(inetgrator)
}
