preprocess.dnam <- function(rawDnam, rawDnamSampleInfo=NULL, savePath, 
    ...
    message.if(me=cat("Preprocessing dnam started at:", 
    ...
    if(inherits(rawDnam, "matrix")){
        message.if(me=cat("rawDnam is the beta matrix!!\n"), verbose=verbose)
    ...
    message.if(me=cat(sum(chrNA),"loci with no chromosome assignment were removed.\n"), 
               verbose=verbose-1)
    ...
    message.if(me=cat(sum(tooManyNAs),"loci with more than 50% NAs were removed.\n"), 
               verbose=verbose-1)
    ...
    message.if(me=cat(sum(!toKeep1),"non-CpG loci were removed.\n"),
               verbose=verbose-1)
    ...
    message.if(me=cat(sum(!toKeep2),"loci with SNP enrichment were removed.\n"),
               verbose-1)
    ...
    if(sum(is.na(SummarizedExperiment::assay(subData4)))/length(SummarizedExperiment::assay(subData4))>0.01){
        ...
        message.if(me=cat(length(nas), "NAs in",
    ...
    message.if(me=cat("Processing DNA methylation data took:", timeTaken, 
