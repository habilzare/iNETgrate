download.data <- function(dataProject, savePath, doDnam=TRUE, doExpr=TRUE,
    ...
    ## Timing:
    starTime <- Sys.time()
    message.if(me=cat("Downloading started at:", format(starTime, usetz=TRUE),"\n"), 
               verbose=verbose-2)
    message.if(me=cat("Data are saved at:", savePath, "\n"), verbose=verbose-2)
    ...
    if(doDnam){
        ...
        result[["rawDnam"]] <- Data 
        dnamEndTime <- Sys.time()
        message.if(me=cat("Downloading DNA methylation data finished at:", 
    ...
    if(doClinical){
        ...
        clinicalEndTime <- Sys.time()
        message.if(me=cat("Downloading clinical data finished at:",
    ...
    if(doExpr){
        ...
        exprEndTime <- Sys.time()
        message.if(me=cat("Downloading expression data finished at:",
    ...
    if(doMirna){
        ...
        message.if(me=cat("Downloading miRNA data finished at:",
