compute.distance.to.tss <- function(usefuLoci, locus2oneGene,
    ...
    ## This function is used in plotLoci.gene().
    ...
    result <- list()
    starTime <- Sys.time()
    message.if(cat("Computing distance to TSS for", length(usefuLoci), "genes started at:", 
                   format(starTime, usetz=TRUE), "\n"), verbose=verbose-3)
    message.if(verbose=verbose, cat("Estimated time to complete:",
    ...
    message.if(cat("Number of NAs in toClosesTss:", length(nas), "\n"),
               verbose=verbose-1)    
    ...
    if(length(notFound)>0)
        warning(paste(length(notFound),"gene symbols were not found in compute.distance.to.tss!"))
    result[["timeTaken"]] <- timeTaken
    message.if(cat("Computing distance to TSS for took:", timeTaken,
