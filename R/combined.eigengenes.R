combined.eigengenes <- function(genExpr=NULL, eigenloci=NULL, netPath, geNames,
    ...
    if(length(intersect(unique(Labels), c(Label1, Label2)))!=2)
        stop(paste("There are no patients labeled as:", Label1, "and/or",
                   Label2))
    if(is.null(geNames))
    ...
    if(!is.null(eigenlociS)& !is.null(genExprS)){
        ...
        if(length(patients) < totalNum)
            warning(paste("Computing combined eigengene is possible for only",
    ...
    for(mu in mus){ ##mu values
        ...
        for(ind in seq_along(lCombine)){ ##e.g., lCombine=c("e"=0, "m"=1, "em0.6"=0.6)
            ...
            combinedValue <- combined.values(genExpr=genExprS, eigenloci=eigenlociS,
            ...
            if(length(naGenes) > 1){
                if(!doIgnoreNas){
                    stop(paste("There are", length(naGenes), "genes with NA values!"))
                } else {
                    Data1 <- Data1[,!(colnames(Data1) %in% naGenes)]
                    warning(paste(length(naGenes), "genes with NA values were removed."))
                }
            }
