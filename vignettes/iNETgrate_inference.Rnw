...
the package can be installed by the following command in Unix:
\newline
$\\$
...
\Rfunction{make.network} function.
...
tcgaAML <- download.data(dataProject="TCGA-LAML", savePath=".")
@
...
and customize  the individual functions such as \Rfunction{make.network} and 
\Rfunction{combined.eigengenes}.
...
resPath <- file.path(getwd(), "iNETgrateRes") ## the result path
...
\Rfunction{clean.allData} function.
...
riskCatCol <- "acute_myeloid_leukemia_calgb_cytogenetics_risk_category"
riskFactorCol <- "cytogenetic_abnormalities"
cleanedToy <- clean.allData(genExpr=toyRawAml$genExpr,
...
with survival data and vital status. The \Rfunction{filter.low.cor} function computes these
...
methylation profiles using \Rfunction{compute.union}.
The above--mentioned filtering and combining steps  
can be performed using the function \Rfunction{elect.genes}, which is a wrapper for
\Rfunction{filter.low.cor} and \Rfunction{compute.union} functions.
...
The output of \Rfunction{elect.genes} is a list of \Rcode{5} components including
...
methylation profiles. This union gene set is further used to make network and
...
Now, our data is ready to make a network using \Rfunction{make.network}. 
...
and then the best value will be determined in later steps of the survival analysis.

<<make_network, eval=TRUE>>=
eigenloci <- computedEloci$eigenloci
madeNet <- make.network(genExpr=cleaned$genExpr, eigenloci=eigenloci,
...
e1 <- combined.eigengenes(genExpr=cleaned$genExpr, eigenloci=eigenloci, 
...
<<survival_analysis, fig.width=12, fig.height=8, eval=TRUE>>=
s1 <- survival.analysis(survival=cleaned$survival, 
                        Labels=patientLabel, favRisk="High", 
                        subSet="Int", mus=0.6, 
...
the \Rfunction{best.inetgrator} can be used to extract the weights in an organized list,
which then can be used by the \Rfunction{infer.eigengenes} function. 

<<inetgrator, eval=FALSE>>=
inetgrator <- best.inetgrator(bestPvalues=s1$bestPvalues, 
